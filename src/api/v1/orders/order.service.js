...
const updateOrderStatus = async ({ orderId, status, paymentStatus, paymentDetails, verifiedAmount, notes = '' }) => {
  const session = await mongoose.startSession();
  session.startTransaction();
...
const adminAssignDriver = async (orderId, driverIdToAssign, adminId, adminRole) => {
  ...
  try {
    ...
    const populatedOrder = await getOrder(orderId, { id: adminId, role: 'admin' });
