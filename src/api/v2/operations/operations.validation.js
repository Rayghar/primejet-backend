...
const addPlantSchema = Joi.object({
  ...
  status: Joi.string()
    .valid('Operational', 'Maintenance', 'Offline', 'Warning')
    .default('Operational')
    .description('Current operational status of the plant.'),
  ...
  location: Joi.object({
    ...
    .optional()
    .description('Geographical location details of the plant.'),
...
// ✅ IMPORTANT FIX:
// Your controller supports BOTH:
// - Plant.findOne({ id: plantId })   (string id)
// - and Plant.findById(plantId)      (ObjectId)
// So validation must allow both.
const plantIdParamSchema = Joi.object({
  plantId: Joi.string().min(3).max(64).required().description('Plant id (string id or Mongo ObjectId).'),
});

// Maintenance log schema
const addMaintenanceLogSchema = Joi.object({
  type: Joi.string()
    .valid('Routine', 'Emergency', 'Repair', 'Upgrade', 'Inspection', 'Other')
    .required()
  ...
  status: Joi.string()
    .valid('Scheduled', 'In Progress', 'Completed', 'Canceled')
    .default('Scheduled')
    .description('Status of the maintenance.'),
});

// ✅ NEW: Stock-In create schema
const addStockInSchema = Joi.object({
  branchId: Joi.string().min(3).required().description('Branch/Plant id (string or ObjectId).'),
  supplier: Joi.string().min(2).max(100).required(),
  purchaseDate: Joi.date().iso().required(),
  quantityKg: Joi.number().min(0.01).required(),
  costPerKg: Joi.number().min(0.01).required(),
  targetSalePricePerKg: Joi.number().min(0.01).required(),

  // optional payment fields used by GL & financials
